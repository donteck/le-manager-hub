...
<?php if(!empty($identity['id'])):?><section class="lmh-member-card" aria-label="Le Manager digital membership card"><div class="lmh-kicker">Le Manager Member</div><h2><?php echo esc_html($user->display_name);?></h2><p><strong>LMID</strong> <?php echo esc_html(substr($identity['id'],0,3).' '.substr($identity['id'],3,3).' '.substr($identity['id'],6,3));?></p><p><?php echo esc_html(strtoupper($identity['level']));?> · <?php echo esc_html(strtoupper($identity['status']));?> · Member since <?php echo esc_html($identity['since']);?></p><small>MEMBERSHIP ID — NOT A PAYMENT CARD</small><div class="lmh-profile-actions" style="margin-top:14px"><a class="lmh-text-link" href="<?php echo esc_url(LMH_Backend::verification_url($user->ID));?>" target="_blank" rel="noopener">VERIFY MEMBERSHIP →</a><a class="lmh-text-link" href="<?php echo esc_url(LMH_Backend::recruitment_url($user->ID));?>" target="_blank" rel="noopener">MY FAN RECRUITMENT LINK →</a></div></section><?php endif;?><p><?php echo esc_html('LM '.(get_user_meta($user->ID,'lmh_fan_level',true)?:'Fan'));?> · <a href="<?php echo esc_url(get_edit_profile_url());?>">Edit account</a> · <a href="<?php echo esc_url(wp_logout_url(home_url('/')));?>">Sign out</a></p><section id="squad" class="lmh-squad"><h2>Le Manager Event Squad</h2><p>Stay part of the community that shows up for music.</p><button class="lmh-btn" id="squad-button" data-joined="<?php echo get_user_meta($user->ID,'lmh_event_squad',true)==='joined'?'true':'false';?>"><?php echo get_user_meta($user->ID,'lmh_event_squad',true)==='joined'?'LEAVE EVENT SQUAD':'JOIN EVENT SQUAD';?></button><p id="squad-status" role="status"></p></section><h2>Artists you follow</h2><div class="lmh-grid"><?php $ids=array_filter(array_map('intval',(array)get_user_meta($user->ID,'lmh_following',true)));$shown=0;foreach($ids as $id)if(get_post_status($id)==='publish'&&get_post_type($id)==='lmh_artist'){$shown++;echo '<a class="lmh-card" href="'.esc_url(get_permalink($id)).'">'.esc_html(get_the_title($id)).'</a>';}if(!$shown)echo '<p>Discover an artist and select Follow to add them here.</p>';?></div><p><a href="<?php echo esc_url(lmh_url('directory'));?>">Discover artists →</a></p><section class="lmh-profile-builder">
