...
final class LMH_Backend {
  ...
  public static function qr_routes() {
    ...
    if(isset($_GET['lmh_verify'])){
      ...
      if($uid&&$stored&&hash_equals($stored,$token)){$u=get_user_by('id',$uid);$m=self::member_identity($uid);$mb=self::member_benefits($uid);$active=$m['status']==='active';echo '<div class="lmh-verification-card '.($active?'is-active':'is-inactive').'"><div class="lmh-verify-status">'.($active?'ACTIVE MEMBERSHIP':'MEMBERSHIP NOT ACTIVE').'</div><h1 class="lmh-title">Membership Verified</h1><p><strong>'.esc_html($u?$u->display_name:'Le Manager Member').'</strong></p><p class="lmh-verify-lmid">LMID '.esc_html(substr($m['id'],0,3).' '.substr($m['id'],3,3).' '.substr($m['id'],6,3)).'</p><p>Level '.esc_html($m['level_number']).' — '.esc_html(strtoupper(str_replace('_',' ',$m['level']))).'</p><hr><h3>Partner Discount Eligibility</h3>'.($active&&$mb['current']['discount']?'<p class="lmh-verify-discount">'.esc_html($mb['current']['discount']).'</p>':'<p>No level-specific partner discount is currently configured.</p>').'<p><small>Verify this screen directly from the member QR. LMID is an identifier, not a password or payment credential.</small></p></div>';}else{echo '<div class="lmh-verification-card is-inactive"><div class="lmh-verify-status">NOT VERIFIED</div><h1 class="lmh-title">Unable to Verify</h1><p>This membership credential is invalid or no longer active. Do not apply a Le Manager member discount from this credential.</p></div>';}
