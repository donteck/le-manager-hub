...
final class LMH_Backend {
  ...
  public static function init() {
    ...
    add_action('admin_post_lmh_edit_partner',[__CLASS__,'admin_edit_partner']);
  ...
  public static function partner_redemption_history($partner_id,$limit=20) {
    $partner_id=sanitize_key($partner_id);$log=(array)get_option('lmh_partner_redemptions',[]);$out=[];
    for($i=count($log)-1;$i>=0&&count($out)<max(1,absint($limit));$i--){$row=$log[$i];if(($row['partner_id']??'')!==$partner_id)continue;$u=get_user_by('id',(int)($row['user_id']??0));$out[]=['member'=>$u?$u->display_name:'Member #'.absint($row['user_id']??0),'level'=>(int)($row['level']??1),'at'=>sanitize_text_field($row['at']??'')];}
    return $out;
  }
  ...
  public static function admin_save_partner() {
    ...
    $name=sanitize_text_field($_POST['name']??'');if(!$name){wp_safe_redirect(admin_url('admin.php?page=lmh-control#partner-network'));exit;}
  ...
  public static function admin_edit_partner() {
    if(!current_user_can('manage_options')) wp_die('Not allowed.');$id=sanitize_key($_POST['partner_id']??'');check_admin_referer('lmh_edit_partner_'.$id);
    $partners=self::partners();if(!$id||empty($partners[$id])) wp_die('Partner not found.');
    $pt=$partners[$id];$pt['name']=sanitize_text_field($_POST['name']??$pt['name']);$pt['category']=sanitize_text_field($_POST['category']??'');$pt['location']=sanitize_text_field($_POST['location']??'');$pt['website']=esc_url_raw($_POST['website']??'');$pt['offer']=sanitize_textarea_field($_POST['offer']??'');$pt['levels']=array_values(array_filter(array_map('absint',(array)($_POST['levels']??[])),fn($n)=>$n>=1&&$n<=7));$partners[$id]=$pt;
    update_option('lmh_partner_network',$partners,false);wp_safe_redirect(admin_url('admin.php?page=lmh-control#partner-network'));exit;
  }
  ...
  public static function control_center() {
    ...
    echo '<div class="wrap"><h1>Le Manager Control Center</h1><p>Manage the professional network, verification, membership growth and booking workflow.</p><nav class="nav-tab-wrapper lmh-admin-nav" style="position:sticky;top:32px;z-index:20;background:#f0f0f1;padding-top:8px"><a class="nav-tab" href="#smart-qr">Smart QR</a><a class="nav-tab" href="#each-one-bring-one">Network</a><a class="nav-tab" href="#membership-levels">Membership</a><a class="nav-tab" href="#qualification-rules">Qualifications</a><a class="nav-tab" href="#ambassadors">Cards</a><a class="nav-tab" href="#level-benefits">Benefits</a><a class="nav-tab" href="#partner-network">Partners</a></nav><script>document.addEventListener("DOMContentLoaded",function(){const nav=document.querySelector(".lmh-admin-nav");if(!nav)return;const links=[...nav.querySelectorAll("a[href^=\\\"#\\\"]")];const items=links.map(a=>({a:a,el:document.querySelector(a.getAttribute("href"))})).filter(x=>x.el);const set=a=>links.forEach(x=>x.classList.toggle("nav-tab-active",x===a));links.forEach(a=>a.addEventListener("click",()=>set(a)));if("IntersectionObserver"in window){const o=new IntersectionObserver(es=>{const v=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(v){const i=items.find(x=>x.el===v.target);if(i)set(i.a)}},{rootMargin:"-20% 0px -65% 0px",threshold:[0,.2,.5]});items.forEach(x=>o.observe(x.el));}if(location.hash){const a=links.find(x=>x.getAttribute("href")===location.hash);if(a)set(a);}});</script>';
    ...
    if($partners){echo '<table class="widefat striped"><thead><tr><th>Partner</th><th>Category</th><th>Location</th><th>Offer</th><th>Levels</th><th>Usage</th><th>Status / Actions</th></tr></thead><tbody>';foreach($partners as $pid=>$pt){echo '<tr><td><strong>'.esc_html($pt['name']).'</strong><details style="margin-top:8px"><summary style="cursor:pointer">Edit Partner</summary><form method="post" action="'.esc_url(admin_url('admin-post.php')).'" style="min-width:320px;padding:10px 0"><input type="hidden" name="action" value="lmh_edit_partner"><input type="hidden" name="partner_id" value="'.esc_attr($pid).'">'.wp_nonce_field('lmh_edit_partner_'.$pid,'_wpnonce',true,false).'<p><input required class="widefat" name="name" value="'.esc_attr($pt['name']).'" placeholder="Partner name"></p><p><input class="widefat" name="category" value="'.esc_attr($pt['category']).'" placeholder="Category"></p><p><input class="widefat" name="location" value="'.esc_attr($pt['location']).'" placeholder="Location"></p><p><input class="widefat" type="url" name="website" value="'.esc_attr($pt['website']).'" placeholder="Website"></p><p><textarea class="widefat" rows="3" name="offer" placeholder="Offer">'.esc_textarea($pt['offer']).'</textarea></p><p><strong>Eligible:</strong> ';foreach(self::MEMBER_LEVELS as $ln=>$ls)echo '<label style="margin-right:7px"><input type="checkbox" name="levels[]" value="'.esc_attr($ln).'" '.checked(in_array($ln,$pt['levels'],true),true,false).'>L'.esc_html($ln).'</label>';echo '</p><button class="button button-primary">Update Partner</button></form></details></td><td>'.esc_html($pt['category']).'</td><td>'.esc_html($pt['location']).'</td><td>'.nl2br(esc_html($pt['offer'])).'</td><td>'.esc_html(implode(', ',array_map(fn($n)=>'L'.$n,$pt['levels']))).'</td><td>'.esc_html(self::partner_redemption_stats($pid)['redemptions']).' uses<br><small>'.esc_html(self::partner_redemption_stats($pid)['unique_members']).' members</small><details style="margin-top:7px"><summary style="cursor:pointer">Recent usage</summary><div style="min-width:230px;padding-top:7px">';$rh=self::partner_redemption_history($pid,10);if($rh){foreach($rh as $rr)echo '<div style="padding:6px 0;border-bottom:1px solid #eee"><strong>'.esc_html($rr['member']).'</strong> · L'.esc_html($rr['level']).'<br><small>'.esc_html($rr['at']).'</small></div>';}else echo '<small>No recorded usage yet.</small>';echo '</div></details></td><td><form method="post" action="'.esc_url(admin_url('admin-post.php')).'" style="display:inline-flex;gap:6px;align-items:center"><input type="hidden" name="action" value="lmh_partner_status"><input type="hidden" name="partner_id" value="'.esc_attr($pid).'">'.wp_nonce_field('lmh_partner_status_'.$pid,'_wpnonce',true,false).'<select name="status"><option value="active" '.selected($pt['status'],'active',false).'>Active</option><option value="inactive" '.selected($pt['status'],'inactive',false).'>Inactive</option></select><button class="button">Save</button></form><form method="post" action="'.esc_url(admin_url('admin-post.php')).'" style="display:inline-block;margin-left:6px" onsubmit="return confirm(\'Delete this partner?\')"><input type="hidden" name="action" value="lmh_delete_partner"><input type="hidden" name="partner_id" value="'.esc_attr($pid).'">'.wp_nonce_field('lmh_delete_partner_'.$pid,'_wpnonce',true,false).'<button class="button button-link-delete">Delete</button></form></td></tr>';}echo '</tbody></table>';}else echo '<p>No partners have been added yet.</p>';echo '</div>';
