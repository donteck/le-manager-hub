<?php
/**
 * Plugin Name: Le Manager Hub Core
...
final class LMH_Core {
  ...
  public static function register_content() {
    ...
    self::cpt('lmh_booking','Booking','Bookings',false);
  ...
  public static function me() {
    ...
    return ['id'=>$u->ID,'name'=>$u->display_name,'email'=>$u->user_email,'roles'=>$u->roles,
      'following'=>array_values(array_map('intval',(array)get_user_meta($u->ID,'lmh_following',true)))];
  }
  ...
  public static function create_booking(WP_REST_Request $r) {
    ...
    if(!$talent) return new WP_Error('missing_talent','Talent/professional is required.',['status'=>400]);
    $id=wp_insert_post(['post_type'=>'lmh_booking','post_status'=>'private',
      'post_title'=>'Booking request '.current_time('Y-m-d H:i:s'),'post_author'=>get_current_user_id()]);
    if(is_wp_error($id)) return $id;
  ...
  public static function follow_artist(WP_REST_Request $r) {
    ...
    if(get_post_type($aid)!=='lmh_artist') return new WP_Error('not_artist','Artist not found.',['status'=>404]);
    $following=(array)get_user_meta($uid,'lmh_following',true);
    $following=array_values(array_unique(array_map('intval',$following)));
...
LMH_Core::init();
register_activation_hook(__FILE__,['LMH_Core','activate']);
register_deactivation_hook(__FILE__,['LMH_Core','deactivate']);
